...
/// be keyed by backend name: every backend has a non-blank name, and no two backends share a trimmed name
/// (compared case-insensitively, as the routing layer keys them). These are exactly the conditions the apply path
/// throws an <see cref="ArgumentException"/> for, so the editor consults this validator to block Apply client-side
/// rather than letting the call throw. Every other rule (URL shape, key length, provider support, per-model rules)
/// is a domain rule left to the recycle's dry-run, which reports it as a graceful rejection.
/// </summary>
/// <remarks>
...
static class DesiredStateStructuralValidator
{
	...
	/// duplicate trimmed backend names), or <see langword="null"/> when the draft is structurally sound, empty, or
	/// not yet loaded.
	/// </returns>
