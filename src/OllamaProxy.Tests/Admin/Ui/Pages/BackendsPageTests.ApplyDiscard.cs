...
public sealed partial class BackendsPageTests
{
	...
	/// </summary>
	[Fact]
	public void SyncAsync_WhenValidationRejected_ShowsRejectionReasons()
	{
		// Arrange
		FakeAdminModelService service = new()
		{
			StateFactory = static () => CreateDraft(CreateBackend("primary")),
			ApplyHandler = static _ => ApplyResult.ValidationRejected(["Base URL is invalid.", "API key is required."])
		};
		(IRenderedComponent<Backends> component, FakeAdminModelService _) = RenderBackends(service);
		...
		Assert.Equal("Base URL is invalid.", reasons[0].TextContent);
		Assert.Equal("API key is required.", reasons[1].TextContent);
	}
	...
	/// </summary>
	[Fact]
	public void SyncAsync_WhenWriteFailed_ShowsWriteFailureMessage()
	{
		// Arrange
		FakeAdminModelService service = new()
		{
	...
	/// </summary>
	[Fact]
	public void SyncAsync_WhenApplySucceeds_ShowsSuccessBannerAfterReload()
	{
		// Arrange
		FakeAdminModelService service = new()
		{
	...
	/// </summary>
	[Fact]
	public void SyncAsync_WhenValidationRejected_KeepsDirtyBanner()
	{
		// Arrange
		FakeAdminModelService service = new()
		{
